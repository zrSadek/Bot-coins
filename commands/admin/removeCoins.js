...
module.exports = {
    name: "removeMoney",
    category: "admin",
    aliases: ["removeArgent", "argentRemove", "removeMoney", "moneyRemove"],
    description: "removeMoney",
    usage: "`.removeArgent <@mention> <coins>`",
    run: async (RosaCoins, message, args) => {
        ...
        if (!isNaN(args[1])) {
            if (parseFloat(args[1]) > 0) {
                ...
                message.channel.send(`Tu viens d'enlever **${args[1]}** Coins à ${user}.`);
            }else
